...
url = "https://#{base_url}/#{json_url}?api_key=#{api_key}&user_id=current&note=#{ARGV[0]}"
uri = URI(url)
...
build_entries = Nokogiri::XML::Builder.new { |xml|
	xml.items {
		time_entries_json.each { |time_entry|
			...
			xml.item('uuid' => 'string', 'autocomplete' => "https://#{@base_url}/time_entries/#{time_entry['time_entry']['id']}", 'arg' => "#{base_url}", 'type' => 'string') {
